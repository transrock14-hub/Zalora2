/**
 * Rebuild Women's Bags — ONE product each (no angle clones).
 * Reclaims DJ/Escuela bags currently sitting in Accessories / Lifestyle.
 *
 * Run: npx tsx scripts/rebuild-women-bags-store-images.ts
 */
...
async function loadCatalog(): Promise<Item[]> {
  ...
  const push = async (p: {
    ...
    if (/test|^string$|arie bagas/i.test(name)) return
    const good: string[] = []
  ...
  const dj = (await (await fetch('https://dummyjson.com/products/category/womens-bags')).json()) as {
    products: any[]
  }
  ...
  for (const p of esc || []) {
    ...
    if (!/bag|handbag|purse|tote|clutch|luggage|backpack/i.test(name)) continue
    await push({
      name,
      description: p.description || name,
  ...
  // Skip FakeStore Fjallraven — reserved for men's bags later
  return items
}

async function reclaimFrom(slug: string, keys: Set<string>, imgs: Set<string>) {
  ...
  for (let i = 0; i < ids.length; i += 40) {
    ...
    const { data: pi } = await supabase
      ...
      .eq('isPrimary', true)
    for (const r of pi || []) {
      if (imgs.has(r.url)) byImg.push(r.productId)
    }
  }
...
async function main() {
  ...
  if (!cat) throw new Error('women-bags missing')
  ...
  console.log(`Unique women's bags: ${catalog.length}`)
  if (catalog.length < 4) throw new Error('too few')

  const keys = new Set(catalog.map((c) => c.name.toLowerCase()))
  const imgs = new Set(catalog.flatMap((c) => c.images))

  const fromAcc = await reclaimFrom(ACC_SLUG, keys, imgs)
  const fromLife = await reclaimFrom(LIFE_SLUG, keys, imgs)
  console.log(`Reclaimed from Accessories ${fromAcc}, Lifestyle ${fromLife}`)
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    const { data: product, error } = await supabase
      ...
      .insert({
        ...
        categoryId: cat.id,
        name: item.name,
        slug: `wb-${slugify(item.name)}-${runTag}-${inserted + 1}`,
        ...
        salePrice: saleNorm,
        costPrice: wholesale,
        sku: `WB-${String(inserted + 1).padStart(3, '0')}-${runTag}`,
  ...
  const accLeft = await supabase
    .from('products')
    .select('id', { count: 'exact', head: true })
    .eq('categoryId', (await supabase.from('categories').select('id').eq('slug', ACC_SLUG).single()).data!.id)
    .eq('status', 'PUBLISHED')
  ...
  writeFileSync(join(process.cwd(), 'catalog', 'women-bags-store-images.csv'), preview.join('\n'))

  console.log("\n=== Women's Bags rebuild ===")
  ...
  console.log('Accessories left ~', accLeft.count, 'Lifestyle ~', lifeLeft.count)
